...
model = dict(
    depth_loss_type="none",  # v1 original loss v2: 我修改的 none:采用真值不计算loss
    type='BEVDepthTCAuxiliary',
    ...
    auxiliary_pts_bbox_head=dict(
        type='CenterHead',
        in_channels=numC_Trans,  # concat bev_structure_reg_head
        tasks=[
            dict(num_class=1, class_names=['car']),
            dict(num_class=2, class_names=['truck', 'construction_vehicle']),
            dict(num_class=2, class_names=['bus', 'trailer']),
    ...
    train_cfg=dict(
        pts=dict(
            ...
            auxiliary_weight = 0.125,
            # depth_loss="gt", #v1 original loss v2: 我修改的 gtv1,gtv2:采用真值不计算loss
        object_temporal_consistance_loss=dict(weight=1, before_fusion=False, use_half_feature=False, lidar_guided=False,bilinear_interpolate=True)),
    test_cfg=dict(
        pts=dict(
            pc_range=point_cloud_range[:2],
...
train_pipeline = [
    ...
    dict(type='BEVObjectCornerAnnotation', grid_config=grid_config, point_cloud_range=point_cloud_range,lidar_guided=False),
    dict(type='ObjectNameFilter', classes=class_names),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d',
                                 'gt_depth', 'corner_relation', 'corner_mask', 'sweeps_gt_boxes', 'sweeps_gt_labels'])
    # dict(type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d',
...
custom_hooks = [
    dict(
        ...
        init_updates=10560,
        priority='NORMAL',
        # resume="work_dirs/0529_mini_noTC_gtdepthv2/epoch_15_ema.pth"
    ),
    dict(
